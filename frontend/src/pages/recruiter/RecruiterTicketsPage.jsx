...
export default function RecruiterTicketsPage() {
  ...
  const fetchTickets = useCallback(async () => {
    try {
      ...
      const data = await getRecruiterTickets({
        page,
        status: statusFilter,
      });
      ...
      console.error("Failed to load recruiter tickets", err);
  ...
  const handleCreateTicket = async (event) => {
    ...
    try {
      ...
      console.error("Failed to create recruiter ticket", err);
    ...
    <div className="space-y-6">
      <section className="space-y-1">
        ...
        <p className="text-sm text-gray-600">
          Create and track your support conversations.
        </p>
      </section>
            ...
            <tbody>
              {tickets.map((ticket) => (
                <tr key={ticket.id} className="border-t border-gray-200">
                  ...
                  <td className="px-4 py-3 text-sm">
                    <button
                      onClick={() => navigate(`/recruiter/tickets/${ticket.id}`)}
