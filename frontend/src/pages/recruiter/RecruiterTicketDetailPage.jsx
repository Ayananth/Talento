...
import {
  getRecruiterTicketDetail,
  replyRecruiterTicket,
...
export default function RecruiterTicketDetailPage() {
  const { id } = useParams();
  ...
  const fetchTicket = useCallback(async () => {
    try {
      setLoading(true);
      setLoadError("");
      const data = await getRecruiterTicketDetail(id);
      setTicket(data);
    } catch (error) {
      console.error("Failed to load recruiter ticket", error);
  ...
  const handleReplySubmit = async (event) => {
    ...
    try {
      setReplying(true);
      await replyRecruiterTicket(id, cleanMessage);
      setReplyText("");
  ...
  if (loadError) {
    return (
      <div className="space-y-4">
        ...
        <button
          onClick={() => navigate("/recruiter/tickets")}
