...
import { getMyProfile, getMyResumes, applyToJob } from "../../../../apis/jobseeker/apis";
...
export default function ApplyJobModal({ open, onClose, jobId, onApplied }) {
  ...
  const prefillCandidateDetails = async () => {
    try {
      ...
      setExperience(
        profile.experience_years === 0 || profile.experience_years
          ? String(profile.experience_years)
          : ""
      );
      setCurrentRole(profile.headline || "");
      setCurrentSalary(
        profile.current_salary === 0 || profile.current_salary
          ? String(profile.current_salary)
          : ""
      );
            ...
            <div className="space-y-4">
              ...
              <label className="flex gap-3 p-4 border rounded-xl cursor-pointer hover:border-blue-400 transition">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-sm text-slate-500">Select from resumes already uploaded</p>
              ...
              <label className="flex gap-3 p-4 border rounded-xl cursor-pointer hover:border-blue-400 transition">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-sm text-slate-500">Redirects to your profile resume section</p>
                </div>
              </label>

              {fieldErrors?.resume && <p className="text-sm text-red-600">{fieldErrors.resume[0]}</p>}
            </div>
          )}
            ...
            <>
              <div className="space-y-2">
                ...
                <div className="flex justify-between text-xs text-slate-500">
                  <span>{coverLetter.length > 0 && `${coverLetter.length} characters`}</span>
                  <span>Optional</span>
                </div>
              </div>

              <div className="space-y-4">
                <h3 className="text-sm font-semibold text-slate-900">Candidate Details</h3>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label className="text-sm font-medium text-slate-700">Current Salary in LPA (optional)</label>
                  <input
                    type="number"
                    value={currentSalary}
                ...
                <div>
                  ...
                  {fieldErrors?.expected_salary && (
                    <p className="text-sm text-red-600">{fieldErrors.expected_salary[0]}</p>
                  )}
                </div>

                <div className="sm:col-span-2">
                  <label className="text-sm font-medium text-slate-700">Notice Period (optional)</label>
                  <input
                    type="text"
                    value={noticePeriod}
